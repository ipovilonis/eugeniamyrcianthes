...
for (variable in variables_respuesta) {
  ...
  assign(paste0("Tabla_", variable), frutos_fisico %>%
    dplyr::filter(!is.na(get(variable))) %>% # Filtra los datos sin NA para la variable actual
    dplyr::group_by(year, site, phenotype) %>%
    dplyr::summarise(!!sym(variable) := mean(!!sym(variable), .groups = "drop")
              )
  )
...
# 0. Asegurar que los rownames de pca_scores sean tus códigos reales
rownames(pca_scores) <- as.character(pca_scores$phenotype)

# 1. Clustering Jerárquico (HCA)
dist_matrix    <- dist(pca_scores[, c("PC1", "PC2", "PC3")], method = "euclidean")
hclust_result  <- hclust(dist_matrix, method = "ward.D2")

# 2. Cortar en 7 clusters
k_clusters     <- 7
clusters_hca   <- cutree(hclust_result, k = k_clusters)
pca_scores$cluster_hca <- factor(clusters_hca)

# 3. Convertir a dendrograma
dendro_obj <- as.dendrogram(hclust_result)

# 4. Graficar con factoextra, usando los rownames como etiquetas
cluster_dend <- fviz_dend(
  dendro_obj,
  k                   = k_clusters,     # número de clusters
  k_colors            = paleta_7,       # paleta para las líneas
  cex                 = 1.5,            # tamaño global de etiquetas
  lwd                 = 1,              # grosor de líneas
  show_labels         = TRUE,           # usar rownames(dat) como labels
  cex_labels          = 1.2,            # tamaño específico de las labels
  rect                = TRUE,           # dibujar rectángulos
  rect_colors         = paleta_7,       # borde de rectángulos
  rect_lwd            = 1,              # grosor del borde
  rect_lty            = 5,              # tipo de línea del borde
  rect_fill           = FALSE,          # sin relleno interior
  color_labels_by_k   = FALSE           # no colorear labels según cluster
) +
  theme_classic() +
  labs(
    title = "",
...
# 5. Mostrar el dendrograma
print(cluster_dend)

# ggsave("cluster_dend.tiff", plot = cluster_dend, device = "tiff", width = 15, height = 10, units = "in", dpi = 300)
...
# ggsave("gg_pca_fruits_cluster_hcas.tiff", plot = gg_pca_fruits_cluster_hca, device = "tiff", width = 15, height = 10, units = "in", dpi = 300)
